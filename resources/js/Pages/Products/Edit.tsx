...
export default function ProductEdit() {
    const { product, categories, taxSettings } = usePage<Props>().props;
    ...
    const { data, setData, post, processing, errors } = useForm<FormData>({
        ...
        margin: (product as any).margin?.toString() || '0',
    ...
    const duplicateVariant = (index: number) => {
        ...
        newVariants.splice(index + 1, 0, {
            ...variantToCopy,
            id: undefined, // ensure it's treated as a new variant by backend
    ...
    const handleSubmit = (e: React.FormEvent | React.MouseEvent) => {
        ...
        post(`/products/${product.id}`, {
            ...
            onError: () => toast.error('Gagal menyimpan perubahan', { description: 'Periksa kembali data yang diinput.' }),
        });
    };
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Edit Produk</h1>
                            <p className="text-sm font-semibold text-gray-500 mt-0.5">{product.name}</p>
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800/50 p-4 shadow-sm">
                        ...
                        <div className="grid grid-cols-6 gap-x-3 gap-y-2">
                            <div className="col-span-3 md:col-span-1">
                                ...
                                <input type="number" value={data.cost_price} onChange={(e) => handleCostChange(e.target.value)} className={inputClass} />
                                {errors.cost_price && <p className="text-[10px] text-red-500 mt-0.5">{errors.cost_price}</p>}
                            </div>

                            <div className="col-span-3 md:col-span-1">
                                <label className={labelClass}>Margin *</label>
                                <input type="number" value={data.margin} onChange={(e) => handleMarginChange(e.target.value)} className={inputClass} />
                                {errors.margin && <p className="text-[10px] text-red-500 mt-0.5">{errors.margin}</p>}
                            </div>

                            <div className="col-span-6 md:col-span-2">
                                <label className={labelClass}>H. Jual {taxSettings.tax_per_item ? '(inc. ppn)' : ''} *</label>
                                <input
                                    type="number"
                                    value={data.price}
